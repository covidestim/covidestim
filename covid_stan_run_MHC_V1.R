...
polygon(c(1:N_days, N_days:1), c(otcm_interval2[1, ], otcm_interval2[2, N_days:1]), border = mTrsp("navy", 30), col = mTrsp("navy", 
  60))
...
legend("topleft", c("Model: new diagnoses (all)", "Model: new diagnoses (reported)", "Data: new diagnoses (reported)"), 
  col = c("red", "navy", "navy"), lty = c(1, 1, 3), pch = c(NA, NA, 16), pt.cex = 0.8, seg.len = 1.5, lwd = c(2, 1, 
    1), cex = 0.9, bty = "n", ncol = 1)
##### 
...
plot(1:N_days, rowSums(otcm_mu), axes = F, xlab = "", ylab = "", type = "l", lty = "11", col = NA, ylim = c(1, mx * 
  1.1), xlim = c(which(rowSums(rep_tri_conf_cases) > 0)[1], N_days_tot), log = "y")
...
legend("topleft", c("Model: new infections", "Model: new symptomatics", "Model: new hospitalizations", "Model: deaths", 
  "Model: recoveries", "Model: new diagnoses (all)", "Model: new diagnoses (reported)", "Data: new diagnoses (reported)"), 
  col = c("grey40", "red", "forestgreen", "blue", "purple", "brown", "orange", "navy", "navy"), lty = c(rep(1, 8), 
    NA), pch = c(rep(NA, 8), 16), pt.cex = 0.7, seg.len = 1.2, lwd = 2, cex = 0.9, bty = "n", ncol = 2)

########### Incident Outcomes ########### New Infections
par(mfrow = c(2, 3), mar = c(1.2, 2, 0.2, 0.2), oma = c(3.5, 3, 3.5, 0.5))

otcm <- samps[["new_inf"]]
plot(1:N_days_tot, apply(otcm, 2, mean), type = "l", ylim = c(0, quantile(otcm[, N_days_tot], 0.975)) * 1.04, las = 1, 
  main = "", axes = F, xlab = "", ylab = "")
axis(2, las = 1, tcl = -0.07, mgp = c(3, 0.15, 0))
axis(1, las = 1, tcl = -0.07, mgp = c(3, 0.1, 0))
box()
polygon(c(1:N_days_tot, N_days_tot:1), c(apply(otcm, 2, function(x) quantile(x, 0.025)), apply(otcm, 2, function(x) quantile(x, 
  0.975))[N_days_tot:1]), border = "grey82", col = "grey90")
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "New infections per day", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "New symptomatic per day", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "New hospitalizations per day", pos = 4, offset = 0.2, font = 2, 
  cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Deaths per day", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Recoveries per day", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Current asymptomatic", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Current symptomatic, non-hospitalized", pos = 4, offset = 0.2, 
  font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Current hospitalized", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Cumulative dead", pos = 4, offset = 0.2, font = 2, cex = 1.15)
...
for (i in smp) lines(1:N_days_tot, otcm[i, ], col = mTrsp(3, 120))
lines(1:N_days_tot, apply(otcm, 2, mean))
text(0, quantile(otcm[, N_days_tot], 0.975) * 1.01, "Cumulative recovered", pos = 4, offset = 0.2, font = 2, cex = 1.15)

mtext("Count of outcomes (N)", 2, 1, outer = T)
mtext("Current and cumulative outcomes: new infections, symptomatics, hospitalizations, deaths, and recoveries", 3, 
  1, outer = T)
mtext("Days since start", 1, 1, outer = T)

############ Priors vs. Posteriors ###########

par(mfrow = c(3, 4), mar = c(1.2, 1.3, 2, 0.2), oma = c(0.5, 2.5, 0.5, 0.5))

## log_new_inf_0
mu = datList[["pri_log_new_inf_0_mu"]]
sd = datList[["pri_log_new_inf_0_sd"]]
hist(samps[["log_new_inf_0"]], col = "goldenrod", border = F, breaks = 20, axes = F, main = "", xlab = "", ylab = "", 
...
hist(mod, col = "goldenrod", border = F, breaks = 10, axes = F, main = "", xlab = "", ylab = "", xlim = range(c(mod, 
  mu - sd/2, mu + sd/2)), probability = T)
axis(2, las = 1, tcl = -0.07, mgp = c(3, 0.15, 0))
axis(1, las = 1, tcl = -0.07, mgp = c(3, 0.1, 0))
box()
...
hist(mod, col = "goldenrod", border = F, breaks = 20, axes = F, main = "", xlab = "", ylab = "", xlim = range(c(mod, 
  0, mu + sd/2)), probability = T)
axis(2, las = 1, tcl = -0.07, mgp = c(3, 0.15, 0))
axis(1, las = 1, tcl = -0.07, mgp = c(3, 0.1, 0))
box()
zz <- range(c(mod, mu - sd, mu + sd))
...
hist(mod, col = "goldenrod", border = F, breaks = 15, axes = F, main = "", xlab = "", ylab = "", xlim = range(c(mod, 
  qbeta(c(2e-04, 0.9998), mn * ss, (1 - mn) * ss))), probability = T)
axis(2, las = 1, tcl = -0.07, mgp = c(3, 0.15, 0))
axis(1, las = 1, tcl = -0.07, mgp = c(3, 0.1, 0))
box()
zz <- range(c(mod, 0, 1))
