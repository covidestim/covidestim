...
## Running the model 
### Data
We will demonstrate how to use the covidestim paackage using data from the [Massachusetts Department of Public Health](https://www.mass.gov/info-details/archive-of-covid-19-cases-in-massachusetts). 
...
Input data and model run options are summarized into a covidestim configuration with the funciton call [`covidestim()`](\code{?covidestim}). Users must specify the number of days of data in the input data. More information on data formating can be found [here](\code{?input_cases}). Calling `covidestim()` will produce a summary of the model run conditions, priors, and data inputs. 
...
### extracted
`extracted` contains the point estimate for every parameter value in every iteration after warm-up. It is a named list, where every element is an array of posterior samples for a paremeter. All three chains have been mixed. 
...
### summary
`summary` contains the point estimate and quantile-based uncertainty bounds for every parameter in the model. The number in brackets indicates the inex day of the estimate. `summary` also contains information about model fit, including the effective sample size and Rhat estimates. 
...
```
While `summary` can be easily converted to a dataframe, we recommend using the helper functions in covidestim, as described above. 
